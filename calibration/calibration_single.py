...
print("Finishing importing libraries")
########################################################################
#   Read settings file
########################################################################
print("importing ini files")
iniFile = "calib_input.ini"
iniCWatM = "calib_ini.ini"
...
print("set multiprocessing")
use_multiprocessing = int(float(parser.get('DEAP','use_multiprocessing')))

try:
    pool_limit = int(float(parser.get('DEAP','pool_limit')))
except:
    pool_limit = 10000

ngen = int(float(parser.get('DEAP','ngen')))
mu = int(float(parser.get('DEAP','mu')))
lambda_ = int(float(parser.get('DEAP','lambda_')))
...
########################################################################
print("Preparation for calibration")
...
def RunModel(Individual):
	...
	template_xml_new = template_xml_new.replace('%run_rand_id', ".\\")
	...
	if runmodel:
		...
		f.close()
		print("this is the path to ini in .bat: " + os.path.join(directory_run, ModelSettings_template[:-4] + '-Run' + run_rand_id + '.ini'))
		#path2ini=os.path.join(directory_run, ModelSettings_template[:-4] + '-Run' + run_rand_id + '.ini')
		path2ini= ModelSettings_template[:-4] + '-Run' + run_rand_id + '.ini' 
		template_bat_new = template_bat
		template_bat_new = template_bat_new.replace('%run',path2ini)
		#print(template_bat_new)
		runfile = os.path.join(directory_run, RunModel_template[:-4]+run_rand_id)
		print("this is the run directory: " + directory_run)
		print(directory_run,RunModel_template[:-4]+run_rand_id)
		if platform == "win32":
			runfile = runfile + ".bat"
		...
		print("runfile location: " + runfile + " end of path")
		print("This is the path called in the shell: " + os.path.join(currentdir, runfile))
		print("Does the path exists? " + str(os.path.exists(os.path.join(currentdir, runfile))))
		p = Popen(os.path.join(currentdir, runfile), shell=True, stdout=PIPE, stderr=PIPE, bufsize=16*1024*1024)
...
if __name__ == "__main__":
	...
	if bestrun:
		...
		for ii in range(0,len(ParamRanges)):
			template_xml_new = template_xml_new.replace("%"+ParamRanges.index[ii],str(Parameters[ii]))
		template_xml_new = template_xml_new.replace('%run_rand_id', os.path.join(currentdir, directory_run))
		...
		with Path("cwatm_best.ini").open("w") as fout:
			#print(tomlkit.dumps(my_dictionary))
			fout.write(template_xml_new)
		...
		print("this is the run directory: " + directory_run)
		print(RunModel_template[:-4]+run_rand_id)	
		...
		os.chdir(directory_run)
		print("runfile location: " + runfile)
		runfile2 = os.path.join(currentdir, runfile)

		p = Popen(runfile2, shell=True, stdout=PIPE, stderr=PIPE, bufsize=16*1024*1024)
		output, errors = p.communicate()
		print(errors)
		f = open("log"+run_rand_id+".txt",'w')
		content = "OUTPUT:\n"+str(output)+"\nERRORS:\n"+str(errors)
		...
		"""Plot the results and save it to pdf"""
		p = Popen("plotCali_fast_2.bat", shell=True, stdout=PIPE, stderr=PIPE, bufsize=16*1024*1024)
		output, errors = p.communicate()
